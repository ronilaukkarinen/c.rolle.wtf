...
// Get only olohuone temp
$result = $database->query( 'SELECT last(temperature) FROM ruuvi_measurements WHERE time > now() - 1h AND "name" = \'Olohuone\' GROUP BY time(2h), "name" ORDER BY DESC LIMIT 60' );
  ...
  <script>
    ...
    var options = {
      ...
      series: [{
        ...
        data: [
          <?php
          $result = $database->query( 'SELECT last(temperature) FROM ruuvi_measurements WHERE time > now() - 1h AND "name" = \'Olohuone\' GROUP BY time(1m), "name" ORDER BY DESC LIMIT 60' );
          $points = $result->getPoints();
    ...
    var options_desktop = {
      ...
      series: [{
        ...
        data: [
          ...
          $result = $database->query( 'SELECT last(temperature) FROM ruuvi_measurements WHERE time > now() - 12h AND "name" = \'Olohuone\' GROUP BY time(1m), "name" ORDER BY DESC LIMIT 720' );
          $points = $result->getPoints();
